...
# model.py
# Core domain: config, rule matching, colors, animations, and Windows-specific operations.
# I keep all business logic here and avoid any UI concerns.
import os
import time
import json
import math
...
class Animation:
    ...
    _state: Dict[str, dict] = {}  # key -> {"time": float, "current": int}

    @staticmethod
    def _get_state(key: str) -> dict:
        s = Animation._state.get(key)
        if not s:
            s = {"time": 0.0, "current": 0xFF0000}
            Animation._state[key] = s
        return s

    @staticmethod
    def color_for(key: str, anim_type: str, speed: float) -> Optional[int]:
        """I dispatch to the requested animation and return an RGB int, or None."""
        ...
        if anim_type == "rainbow":
            return Animation._rainbow(key, speed)
        if anim_type == "pulse":
            return Animation._pulse(key, speed)
        if anim_type == "fade":
            return Animation._fade(key, speed)
        return None

    @staticmethod
    def _rainbow(key: str, speed: float) -> int:
        """I cycle through RGB corners to create a rainbow effect."""
    ...
    @staticmethod
    def _pulse(key: str, speed: float) -> int:
        """I pulse between red and blue using a sine wave."""
        st = Animation._get_state(key)
        st["time"] += speed * 0.05
    ...
    @staticmethod
    def _fade(key: str, speed: float) -> int:
        """I create a grayscale fade using a sine wave."""
        st = Animation._get_state(key)
        st["time"] += speed * 0.02
        t = (math.sin(st["time"]) + 1) / 2.0
        v = int(255 * t)
        st["current"] = (v << 16) | (v << 8) | v
        return st["current"]
...
def apply_colors_once(config: dict, animated_only: bool = False):
    ...
    def cb(hwnd, _):
        ...
        if not rule:
            rule = next((r for r in config.get("window_rules", []) if (r.get("match") or "").lower() == "global"), None)
        ...
        if anim["type"] != "none":
            # I use a stable key per process to avoid color churn.
            key = f'{process_name}|{anim["type"]}'
            c = Animation.color_for(key, anim["type"], anim["speed"])
            if c is not None:
                r = (c >> 16) & 0xFF
                g = (c >> 8) & 0xFF
                b = c & 0xFF
